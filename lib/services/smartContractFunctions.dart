...
class SmartContractFunction {
  ...
  Future balanceOfNFT(var contractAddr, var nftId) async {
    String abiStringFile =
        await rootBundle.loadString("SmartContract/nft_apparel_v2.abi.json");
    final contract = DeployedContract(
        ContractAbi.fromJson(abiStringFile, 'ERC1155'),
    ...
    final address = userData.walletAddress;
    // var address = await credentials.extractAddress();
    var balance = await ethClient.call(
        contract: contract,
        function: balanceOfNFT,
        params: [EthereumAddress.fromHex(address!), BigInt.from(nftId)]);
    return balance[0];
  }
}
